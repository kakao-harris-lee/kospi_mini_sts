...
class TradingCNNLSTM(nn.Module):
    ...
    def __init__(
        ...
        self.cnn = nn.Sequential(
            # Conv1: input_dim -> cnn_channels[0]
            nn.Conv1d(input_dim, cnn_channels[0], kernel_size=kernel_size, padding=1),
            nn.BatchNorm1d(cnn_channels[0]),
            nn.ReLU(),
            nn.Dropout(dropout),

            # Conv2: cnn_channels[0] -> cnn_channels[1]
            nn.Conv1d(cnn_channels[0], cnn_channels[1], kernel_size=kernel_size, padding=1),
            nn.BatchNorm1d(cnn_channels[1]),
            nn.ReLU(),
            nn.Dropout(dropout),

            # Pooling: seq_len -> seq_len // 2
            nn.MaxPool1d(kernel_size=2, stride=2)
        )

        # LSTM: cnn_channels[1] features, reduced seq_len
        self.lstm = nn.LSTM(
            cnn_channels[1],  # CNN 출력 채널이 LSTM 입력
            hidden_dim,
            num_layers,
            batch_first=True,
            dropout=dropout if num_layers > 1 else 0,
    ...
    def forward(self, x):
        ...
        # 1. Transpose for Conv1d: (batch, features, seq_len)
        x = x.transpose(1, 2)

        # 2. CNN block
        x = self.cnn(x)  # (batch, cnn_channels[1], seq_len // 2)

        # 3. Transpose back for LSTM: (batch, seq_len // 2, cnn_channels[1])
        x = x.transpose(1, 2)
...
def train_model(
    ...
    if model_type == "cnn-lstm":
        model = TradingCNNLSTM(input_dim=input_dim).to(device)
        print(f"Using CNN-LSTM model")
    else:
...
def main():
    ...
    parser.add_argument("--model-type", choices=["cnn-lstm", "lstm"], default="cnn-lstm",
    ...
    # CNN-LSTM specific metadata
    if model_type == "cnn-lstm":
        meta["cnn_channels"] = [32, 64]
        meta["kernel_size"] = 3
