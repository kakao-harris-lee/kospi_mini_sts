...
@app.command("kospi200-index")
def run_kospi200_index_backfill(
    days: int = typer.Option(
        180,
        "--days", "-d",
        help="백필할 일수 (기본: 180일)",
    ...
    """KOSPI 200 지수 과거 데이터 백필"""
    from src.collector.historical import backfill_kospi200_index

    console.print(f"\n[bold]KOSPI 200 지수 과거 {days}일 데이터 백필 시작[/bold]\n")

    try:
        rows = asyncio.run(backfill_kospi200_index(days=days, verbose=verbose))
        console.print(f"\n[green]백필 완료![/green] 총 {rows:,}건 수집")
    except Exception as e:
        console.print(f"[red]에러:[/red] {e}")
        raise typer.Exit(1)
...
@app.command("kospi200-index-today")
def collect_kospi200_index_today(
    verbose: bool = typer.Option(True, "--verbose/--quiet"),
):
    """오늘 KOSPI 200 지수 데이터 수집 (장 마감 후)"""
    from src.collector.historical import collect_today_kospi200_index

    console.print("\n[bold]오늘 KOSPI 200 지수 데이터 수집[/bold]\n")

    try:
        rows = asyncio.run(collect_today_kospi200_index(verbose=verbose))
        if rows > 0:
            console.print(f"\n[green]수집 완료![/green] {rows:,}건")
        else:
            console.print("[yellow]수집된 데이터가 없습니다.[/yellow]")
